...
def video_stream(frame_queue, processed_frame_queue, stop_event):
    ...
    cap = cv.VideoCapture("https://192.168.124.165:8080/video")
    if not cap.isOpened():
...
def age_gender_detection(
    frame_queue, processed_frame_queue, stop_event, ad, age_gender_info, recent_ads, change_ad_flag, interested_start_times, interested_durations
):
    Config.load()  # Load configuration in the child process
    Logger.init()  # Initialize Logger in the child process
    ...
    while not stop_event.is_set():
        try:
            ...
            if predictions:
                ...
                ad_playback(ad.value, recent_ads, change_ad_flag, interested_start_times, interested_durations)
        except queue.Empty:
...
def ad_playback(ad, recent_ads, change_ad_flag, interested_start_times, interested_durations):
...
def main():
    ...
    detection_process = multiprocessing.Process(
        ...
        args=(frame_queue, processed_frame_queue, stop_event, ad, age_gender_info, recent_ads, change_ad_flag, interested_start_times, interested_durations),
    )
