...
class EyeballDetector:
    def __init__(
        ...
        min_confidence=0.5,  # originally 0.998
        reasonable_confidence=0.5,  # originally 0.997
    ...
    def update(self, new_points, timestamp_ms):
        ...
        if self.points_for_eye_center is not None:
            self.points_for_eye_center = np.concatenate(
                (self.points_for_eye_center, new_points), axis=0
            )[-self.points_history_size :]
        else:
            self.points_for_eye_center = new_points

        if (
            len(self.points_for_eye_center) >= self.points_threshold
            and not self.search_completed
        ):
            center, radius, confidence = self._solve_for_sphere(
                self.points_for_eye_center
            )
            ...
            if confidence and confidence > self.current_confidence:
                ...
                if confidence >= self.reasonable_confidence:
                    self.search_completed = (
                        True  # Indicate that the search can be concluded
                    )
    ...
    def _solve_for_sphere(self, points, radius_bounds=(0.015, 0.025)):
        ...
        result = minimize(
            objective,
            np.append(self.eye_center, self.eye_radius),
            args=(points,),
            bounds=bounds,
        )
